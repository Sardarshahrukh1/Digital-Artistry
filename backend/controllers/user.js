...
exports.rejectUser = async (req, res) => {
  ...
  try {
    ...
    res.json({ message: "User rejected successfully", user: user._id });
  } catch (err) {
    res.status(500).json({ message: "Server error" });
  }
};
    ...
    if (req.user.id !== req.params.id) {
      ...
      if (
        ...
        await addNotification({
          title: `Friend request has been requested by ${sender.username}`,
          notificationFor: mongoose.Types.ObjectId(receiver._id),
          notificationOf: mongoose.Types.ObjectId(sender._id),
        });
  ...
  try {
    if (req.user.id !== req.params.id) {
      ...
      if (
        ...
        await addNotification({
          title: `Your request has been cancelled by ${sender.username}`,
          notificationFor: mongoose.Types.ObjectId(receiver._id),
        });
  ...
  try {
    if (req.user.id !== req.params.id) {
      ...
      if (receiver.requests.includes(sender._id)) {
        ...
        await addNotification({
          title: `Friend request has been accepted by ${receiver.username}`,
          notificationFor: mongoose.Types.ObjectId(sender._id),
          notificationOf: mongoose.Types.ObjectId(receiver._id),
        });
...
exports.getFriendsList = async (req, res) => {
  try {
    const user = await User.findById(req.user.id)
      .select("friends")
      .populate("friends", "first_name last_name username _id picture");
